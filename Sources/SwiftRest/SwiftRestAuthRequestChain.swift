...
public extension SwiftRestAuthClient {
    /// Starts a chain-based request for a path.
    nonisolated func path(_ path: String) -> SwiftRestAuthPathBuilder {
        SwiftRestAuthPathBuilder(client: self, path: path)
    }
}

/// Chainable request builder from `auth.path(...)`.
public struct SwiftRestAuthPathBuilder: Sendable {
    private let client: SwiftRestAuthClient
    ...
    init(client: SwiftRestAuthClient, path: String) {
        self.client = client
        self.request = SwiftRestRequest(path: SwiftRestPathUtilities.normalizedPath(path), method: .get)
    ...
    public func path<Segment: SwiftRestPathSegmentConvertible>(_ segment: Segment) -> Self {
        ...
        copy.request.configurePath(
            SwiftRestPathUtilities.joinedPath(copy.request.path, segment.swiftRestPathSegment)
        )
        return copy
    }
    ...
    public func path(url: URL) -> Self {
        paths(SwiftRestPathUtilities.pathSegments(from: url.path))
    }
    ...
    ///
    /// Precedence: per-request token overrides the session store token.
    ...
    public func get() -> SwiftRestAuthPreparedRequest {
        prepared(method: .get)
    }

    /// Prepares a `HEAD` request.
    public func head() -> SwiftRestAuthPreparedRequest {
        prepared(method: .head)
    }

    /// Prepares an `OPTIONS` request.
    public func options() -> SwiftRestAuthPreparedRequest {
        prepared(method: .options)
    }

    /// Prepares a `DELETE` request.
    public func delete() -> SwiftRestAuthPreparedRequest {
    ...
    public func post<Body: Encodable & Sendable>(
        body: Body
    ) throws -> SwiftRestAuthPreparedRequest {
    ...
    private func prepared(method: HTTPMethod) -> SwiftRestAuthPreparedRequest {
        var copy = request
        copy.configureMethod(method)
        return SwiftRestAuthPreparedRequest(
            client: client,
            request: copy,
...
public struct SwiftRestAuthPreparedRequest: Sendable {
    private let client: SwiftRestAuthClient
